...
MIME_TYPE_SCORE_FILE = default_data_dir = os.path.dirname(
    os.path.abspath(__file__)) + '/../../../ckanext-ecportal/data/resource_type_score.json'
MIME_TYPE_SCORE = json.load(open(MIME_TYPE_SCORE_FILE))
...
def _update_task_status(context, data):
    ...
    """
    path = '/data'
    if '/euodp' not in context['site_url']:
        path = ''

    ckan_url = '%s%s' % (context['site_url'], path)
    api_url = '%s/%s' % (ckan_url, 'api/action')
    ...
    if res.status_code == 200:
        ...
        raise CkanError('ckan failed to update task_status, status_code (%s), error %s, %s'
                        % (res.status_code, res.content, api_url))
...
def resource_score(context, data):
    ...
    if not data.get('is_open'):
        ...
        try:
            ...
            url = next((url for url in data['access_url']), None) or next((url for url in data['download_url']), None) or ''
