...
class QAPlugin(p.SingletonPlugin):
    ...
    def notify(self, entity, operation=None):
        ...
        if not isinstance(entity, DistributionSchemaDcatApOp) and not isinstance(entity, DocumentSchemaDcatApOp):
            return
    ...
    def _create_task(self, resource):
        ...
        if isinstance(resource, DistributionSchemaDcatApOp):
            resource_dict = ui_util._transform_resources_to_ui_schema(resource, 'en')
        elif isinstance(resource, DocumentSchemaDcatApOp):
            resource_dict = ui_util._transform_resources_page_foaf_to_ui_schema(resource, 'en')



        resource_dict['is_open'] = resource_dict['is_open']
