...
class QACommand(p.toolkit.CkanCommand):
    ...
    def _package_list(self):
        ...
        """
        path = config.get('ckan.root_path',None)
        self.log.warning(path)
        path = '/' + path.split('/')[-1] if path else ''
        self.log.warning('%s%s' % (config['ckan.site_url'], path))
        ckan_url = '%s%s' % (config['ckan.site_url'], path)
        self.log.warning(ckan_url)
        ...
        if len(self.args) > 1:
            ...
            response = self.make_post(url, {'offset': page, 'limit': limit})
            if not response.ok:
