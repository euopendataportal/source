this.ckan.module('reclinepreview', function (jQuery, _) {
    return {
        options: {
            ...
            recline.Backend.DataProxy.timeout = 100;
            ...
            if (resourceData.datastore_active) {
                ...
                console.log(this.options.i18n.errorLoadingPreview + ': ' + this.options.i18n.errorDataStore);
                showError(this.options.i18n.errorLoadingPreview + ': ' + this.options.i18n.errorDataStore);
            }
        }, initializeDataExplorer: function (dataset) {
            var views = [{id: 'grid', label: 'Grid', view: new recline.View.SlickGrid({model: dataset})}, {
                id: 'graph',
                label: 'Graph',
                view: new recline.View.Graph({model: dataset})
            }, {id: 'map', label: 'Map', view: new recline.View.Map({model: dataset})}];
            var sidebarViews = [{
                id: 'valueFilter',
                label: 'Filters',
                view: new recline.View.ValueFilter({model: dataset})
            }];
