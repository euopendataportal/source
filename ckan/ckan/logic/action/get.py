...
def _group_or_org_list(context, data_dict, is_org=False):
    ...
    if is_org:
        query = model.Session.query(model.Group).select_from(model.Group).join(model.GroupRevision)
    else:
        query = model.Session.query(model.Group)\
            .select_from(model.Group)\
                .join(model.GroupRevision)\
                .outerjoin(model.term_translation_table,
                           _and_(model.Group.title == model.term_translation_table.c.term,
                                 model.term_translation_table.c.lang_code==locale))
        query = query.filter(_or_(model.term_translation_table.c.lang_code==locale,
                                  model.term_translation_table.c.lang_code==None))
    ...
    if q:
        ...
        if is_org:
            ...
            query = query.filter(_or_(
                ...
                model.GroupRevision.title.ilike(q),
                model.GroupRevision.description.ilike(q),
                model.term_translation_table.c.term_translation.ilike(q),
            ))
...
def organization_list_for_user(context, data_dict):
    '''Return the list of organizations that the user can edit.
...
def term_translation_show(context, data_dict):
    ...
    if len(terms) == 0:
        return []
    if isinstance(terms, basestring):
        q = q.where(trans_table.c.term == terms)
    if terms:
    ...
    if 'lang_codes' in data_dict:
        ...
        if isinstance(lang_codes, basestring):
            q = q.where(trans_table.c.lang_code == lang_codes)
        else:
