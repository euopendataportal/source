...
class ApiController(base.BaseController):
    ...
    def action(self, logic_function, ver=None):
        ...
        try:
            ...
            # adding this to properly catch and report errors from datastore
            return_dict.pop('help')
            return_dict['error'] = e.message
            return_dict['orig_statement'] = e.statement
            return_dict['success'] = False
            # CS nasty_string ignore
            log.error('Datastore Error: %r' % str(return_dict))
            return self._finish(422, return_dict, content_type='json')
    ...
    def _get_request_data(cls, try_url_params=False):
        ...
        if request.POST and request.content_type == 'multipart/form-data':
            ...
            try:
                ...
                #Sometimes, for any reason, a '=' is added to the JSON data so that it becomes
                #becomes invalid. This bug's reason could not be found yet, so work around it
                if request_data[len(request_data)-2: len(request_data)] == "}=":
                        request_data = request_data[0:len(request_data)-1]
            except Exception, inst:
