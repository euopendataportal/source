...
class PackageController(base.BaseController):
    ...
    def read(self, id, format='html'):
        ...
        package_type = self._get_package_type(id.split('@')[0])
        context = {'model': model, 'session': model.Session,
                   'user': c.user or c.author, 'for_view': extension != 'rdf',
                   'auth_user_obj': c.userobj}
        data_dict = {'id': id}
    ...
    def _save_edit(self, name_or_id, context, package_type=None):
        ...
        try:
            data_dict = clean_dict(dict_fns.unflatten(
                tuplize_dict(parse_params(request.POST))))
            #allows settings groups from dataset edit page
            if (request.POST.get('groups') and data_dict.get('groups')) or\
                (request.POST.get('domains_eurovoc') and data_dict.get('domains_eurovoc')):
                groups_ids = data_dict.get('groups') if isinstance(data_dict.get('groups'), list) else [data_dict.get('groups')]
                domains_ids = data_dict.get('domains_eurovoc') if isinstance(data_dict.get('domains_eurovoc'), list) else [data_dict.get('domains_eurovoc')]
