...
# script based on https://raw.github.com/okfn/ckan-packaging/master/etc/cron.daily/remove_old_sessions
# Written 2012 by Nils Toedtmann for OKFN <nils.toedtmann@okfn.org>
# adapted to EU ODP by TenForce
...
if [ -z "${DIRS_TO_CLEANUP}" ]; then
  DIRS_TO_CLEANUP="/applications/ecodp/users/ecodp/ckan/var/sessions/"
fi
...
#find /applications/ecodp/users/ecodp/ckan/var/sessions/ -type f  -mtime +7 -delete -print

for dir in ${DIRS_TO_CLEANUP}; do
    ${SUDO} ${FIND_BIN} $dir ${FIND_OPTIONS} -mtime +${REMOVE_OLDER_THAN_MINS} -delete -print
done
