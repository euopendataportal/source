...
$(function () {
    ...
    $('a.js-resource-add').on('click', function (event) {
        ...
        if(type=="visualization") {
            ...
            //$('.resource-add').show();
            var new_resource_details = add_resource_visualization(index);
     ...
     function create_resource_list_item(res_number, type, index) {
        ...
        var new_resource_list_item = $("#"+id_panel + " .templates_visualization>.resource_list_visualization_item_template").clone();
        new_resource_list_item.attr('id', 'dataset-'+index+'-'+type+'-resource-list_visualization-item-' + res_number);
	...
	function add_resource_visualization(index) {

        var details_panel = $('#main-resource-panel_visualization_'+index);

       var activeResource = $("#tabs-visualization_" + index).find("ul li.ui-state-active a");
        var dataset_type = activeResource.data('resource');
        var resource_list = $('#resource-list-visualization-div_' + index);
        var displayIframe = true;

        var res_number;
        if (details_panel.children(".resource-details_visualization[data-resnumber]").size() == 0) {
            res_number = 0;
        } else {
            res_number = parseInt(details_panel.children().last().attr("data-resnumber")) + 1;
        }
        var new_resource_details = initialize_visualization_resource_details(res_number, index);
        ...
        new_resource_details.find("#dataset_" + index + "-resources_visualization__" + res_number + "__release_date").removeClass("hasDatepicker").datepicker({
            dateFormat: 'yy-mm-dd'
        });
        new_resource_details.find("#dataset_" + index + "-resources_visualization__" + res_number + "__modification_date").removeClass("hasDatepicker").datepicker({
            dateFormat: 'yy-mm-dd'
        });
    ...
    function initialize_visualization_resource_details(res_number, index) {
        var default_lang = 'en';
        var dataset_panel = $("div[data-index='"+index+"']");
        var id_panel = dataset_panel.attr("id");
        var new_resource_details = $("#"+id_panel + " .templates_visualization>.resource-details_visualization-template").clone();
        new_resource_details.attr("id", "dataset_"+index+"-resource_details_visualization_"+ res_number);
        ...
        new_resource_details.html(new_resource_details.html().replace(/name="resources_visualization__template/g, "name=\"dataset__"+index+"__resources_visualization__"+res_number));
        new_resource_details.html(new_resource_details.html().replace(/id="resources_visualization__template/g, "id=\"dataset_"+index+"-resources_visualization__"+res_number));


        new_resource_details.find("#dataset_"+index+"-resources_visualization__"+res_number+"__release_date").datepicker({
            dateFormat: 'yy-mm-dd'
        });
        new_resource_details.find("#dataset_"+index+"-resources_visualization__"+res_number+"__modification_date").datepicker({
            dateFormat: 'yy-mm-dd'
        });
        ...
        var title_tag = new_resource_details.find("[name='dataset_"+index+"-resources_visualization__"+res_number+"__title']");
        // TODO : fix this, #resource-list-item not correct anymore
        title_tag.keyup(function (event) {
            if (title_tag.val() != '') {
                $("#dataset_"+index+"-resource-list_visualization-item-" + res_number).find('em').text(title_tag.val());
            } else {
                $("#dataset_"+index+"-resource-list_visualization-item-" + res_number).find('em').text('[new resource]');
            }
        });
        var format_tag = new_resource_details.find("#dataset_"+index+"-resources_visualization__"+res_number+"__format");
        format_tag.append($('.templates_visualization #resource_dropdown_format_template').clone().children());

        var language_tag = new_resource_details.find("#dataset_"+index+"-resources_visualization__"+res_number+"__language");
        ...
        var licence_tag = new_resource_details.find("#dataset_"+index+"-resources_visualization__"+res_number+"__licence");
        licence_tag.append($('.templates_visualization #resource_dropdown_licence_template').clone().children());

        var status_tag = new_resource_details.find("#dataset_"+index+"-resources_visualization__"+res_number+"__status");
