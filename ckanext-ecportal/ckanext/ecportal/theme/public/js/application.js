...
CKAN.View.UrlEditor = Backbone.View.extend({
  ...
  titleChanged:  function() {
    ...
    var index = $('.div_toggle:visible').data('index');
    if (index || index == 0){
      $('#name_'+index).val(slug);
    }else {
...
CKAN.View.Resource = Backbone.View.extend({
  ...
  render: function() {
    ...
    var resource_object = {
        ...
        resourceTypeOptions: [
          ['http://data.europa.eu/euodp/kos/documentation-type/MainDocumentation', CKAN.Strings.documentationMain],
          ['http://data.europa.eu/euodp/kos/documentation-type/RelatedDocumentation', CKAN.Strings.documentationRelated],
          ['http://data.europa.eu/euodp/kos/documentation-type/RelatedWebPage', CKAN.Strings.documentationWebpage],
          ...
          ['http://data.europa.eu/euodp/kos/documentation-type/Visualization', CKAN.Strings.visualization]
        ],
        formatTypeOptions: resource_dropdown,
        formatTypeOptionsMimetype: resource_dropdown};
    ...
    // if there is a resource.format object set, we need to get the mapping and filter out the other values
    if (!jQuery.isEmptyObject(resource_object.resource.format)) {
    	
    	var resource_dropdown_temp = [];
	var resource_mapping_entry = resource_mapping_json[resource_object.resource.format];
	var resource_format_name;
	if (undefined != resource_mapping_entry) {
        	resource_format_name = resource_mapping_entry[2];
    	}
	$.each(resource_mapping_json, function (index, value) {
		if(resource_format_name == value[2]) {
			resource_dropdown_temp.push(value[0]);
		}
    	});
    	
    	$.unique(resource_dropdown_temp);
    	
    	var resource_dropdown_filtered = [];
    	$.each(resource_dropdown_temp, function (index, value) {
    		resource_dropdown_filtered.push(new Array(value, value));
    	});
    	
    	if(!jQuery.isEmptyObject(resource_dropdown_filtered)) {
    		resource_object.formatTypeOptionsMimetype = resource_dropdown_filtered;
    	}
