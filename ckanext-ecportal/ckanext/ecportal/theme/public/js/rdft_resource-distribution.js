...
$(function () {
    ...
    function create_resource_list_item(res_number) {
        var new_resource_list_item = $(".templates_distribution>.resource_list_distribution_item_template").clone();
        new_resource_list_item.attr('id', 'resource-list_distribution-item-' + res_number);
    ...
    function add_resource_distribution(activeResource){
        ...
        var resource_list = $('#resource-list-distribution');
        var new_resource_details = initialize_distribution_resource_details(res_number);
    ...
    function initialize_distribution_resource_details(res_number) {
        var default_lang = 'en';
        var new_resource_details = $(".templates_distribution>.resource-details_distribution-template").clone();
        new_resource_details.attr("id", "resource_details_distribution_"+ res_number);
        ...
        new_resource_details.find("#resources_distribution__"+res_number+"__release_date").datepicker({
            dateFormat: 'yy-mm-dd'
        });
        new_resource_details.find("#resources_distribution__"+res_number+"__modification_date").datepicker({
            dateFormat: 'yy-mm-dd'
        });
        ...
        langs.each(function () {
            var id = $(this).attr("id");
            id = id.replace("__"+res_number+"__", "-" + res_number);
            $(this).attr("id", id);
        ...
        var title_tag = new_resource_details.find("[name='resources_distribution__"+res_number+"__title']");
        // TODO : fix this, #resource-list-item not correct anymore
        title_tag.keyup(function (event) {
            if (title_tag.val() != '') {
                $("#resource-list_distribution-item-" + res_number).find('em').text(title_tag.val());
            } else {
                $("#resource-list_distribution-item-" + res_number).find('em').text('[new resource]');
            }
        });
        var format_tag = new_resource_details.find("#resources_distribution__"+res_number+"__format");
        format_tag.append($('.templates_distribution #resource_dropdown_format_template').clone().children());

        var language_tag = new_resource_details.find("#resources_distribution__"+res_number+"__language");
        ...
        var licence_tag = new_resource_details.find("#resources_distribution__"+res_number+"__licence");
        licence_tag.append($('.templates_distribution #resource_dropdown_licence_template').clone().children());

        var status_tag = new_resource_details.find("#resources_distribution__"+res_number+"__status");
    ...
    $('a.js-resource-add').click(function (event) {
        ...
        if(type=="distribution"){
            ...
            var new_resource_details = add_resource_distribution($(this).data('type'));
    ...
    $(document).on('click', '.resource-lang', function () {
        ...
        if(type=='distribution'){
            ...
            add_translations_inputs(new_lang, "resources_distribution__" + resNumber + "__title", "js-resource-edit-title-field translatable-resource-field", "", "", widgetInput);
            add_translations_inputs(new_lang, "resources_distribution__" + resNumber + "__description", "js-resource-edit-description markdown-input translatable-resource-field", "", "", widgetTextarea);
            // add_translations_inputs(new_lang, "resources_distribution__" + resNumber + "__download_url", "js-resource-edit-download_url long translatable-resource-field", "", "", widgetInput);
