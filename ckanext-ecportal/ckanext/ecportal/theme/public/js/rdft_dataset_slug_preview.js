  ...
  initialize: function () {
    ...
    if (el.val() == '') {
      ...
      el.on('keyup.custom_slug-preview', function (event) {
        var index = $(event.currentTarget).closest('.div_toggle:visible').data('index');
        sandbox.publish('custom_slug-target-changed', this.value+'_'+index);
  ...
  return {
    ...
    initialize: function () {
      ...
      sandbox.subscribe('custom_slug-target-changed', function (value) {
        ...
        if (slug.attr('id') == 'dataset_'+index+'-name') {
          slug.val(value.substring(0, value.lastIndexOf('_'))).trigger('change');
        }
      });
    }
  };
