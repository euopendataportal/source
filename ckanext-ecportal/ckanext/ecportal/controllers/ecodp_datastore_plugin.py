...
class ECODPDatastorePlugin(p.SingletonPlugin):
    ...
    def notify(self, entity, operation=None):
        if not isinstance(entity, DatasetDcatApOp) or self.legacy_mode:
            return
        ...
        if operation == model.domain_object.DomainObjectOperation.changed:
            ...
            if entity.privacy_state == DCATAPOP_PRIVATE_DATASET:
                func = p.toolkit.get_action('datastore_make_private')
            else:
                func = p.toolkit.get_action('datastore_make_public')
            for resource in entity.schema.distribution_dcat.values():
                try:
                    func(context, {
                    ...
                    pass
            for resource in entity.schema.page_foaf.values():
                try:
                    func(context, {
                        'connection_url': self.write_url,
                        'resource_id': resource.uri.split('/')[-1]})
    ...
    def before_show(self, resource_dict):
        ...
        try:
            ...
            resource_id = resource_dict.get('id', None)
            if resource_id is not None:
                row_resource_count = model.Session.execute('''
            select r.resource_count AS resource_count
            FROM resource r
            WHERE r.id =  :resource_id
            ''', {'resource_id': resource_id})
            for resource_count in row_resource_count:
                resource_dict['download_total_resource'] = int(resource_count[0])
